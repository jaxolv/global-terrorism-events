...
media_global_ataques_sucesso as (
    select
        round(sum(case when success then 1 else 0 end)::numeric / count(*), 2) * 100
            as media_perc_sucesso
    from terrorism_act
)
select 
    at.country as pais,
    concat(tas.percentual_sucesso, '%') as percentual_ataques_sucesso
from taxa_ataques_sucesso tas
join ataques_sucesso at on at.country_id = tas.country_id
cross join media_global_ataques_sucesso m
where tas.percentual_sucesso > m.media_perc_sucesso
order by percentual_sucesso asc;
